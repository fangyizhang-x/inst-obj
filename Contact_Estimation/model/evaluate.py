...
from utils import load_data, taxel_projection
import argparse
import yaml
...
def visualize_sample(pred, gt, projected=False):
    ...
    # ax.hist(err_matrix[:,i],10, color='blue',alpha=0.8)
    # ax.set_title('median: '+ str(np.around(np.median(err_matrix[:,i]),3)) + ', max: ' + str(np.around(np.max(err_matrix[:,i]),3)))    
...
#     fig, ax = plt.subplots()
    ...
    ax.set_xticks(np.arange(10), labels=range(1,11))
    ax.set_yticks(np.arange(10), labels=range(1,11))
    plt.colorbar(im, ax=ax)  
    plt.title('Predicted heatmap',y=-0.16)
...
#     fig, ax = plt.subplots()
    ...
    ax.set_yticks(np.arange(10), labels=range(1,11))
    plt.colorbar(im, ax=ax)  
    plt.title('Ground-truth heatmap',y=-0.16)
    ax.xaxis.set_ticks_position('top')
    ax.set_xlabel('X axis')
    ax.set_ylabel('Y axis')
    ax.xaxis.set_label_position('top') 
...
def contact_location_accuracy(pred, gt, thres, result_path):
    ...
    # if len(cc_max_values) > 0:
    #     ind = np.argsort(cc_max_values)[len(cc_max_values)//2]
    # if len(force_errors) > 0:
    #     ind = np.argsort([abs(ele) for ele in force_errors])[len(force_errors)//2]
    #     visualize_sample(proj_pred[ind], proj_gt[ind], projected=True)
    #     plt.draw()
    #     plt.savefig(f"{result_path}/median_sample_{round(force_errors[ind],3)}.png", dpi=400, bbox_inches='tight')
    #     plt.close() 
...
def plot_histgram2d(x, y, fn):
    ...
    for i in range(len(yedges)-1):
        for j in range(len(xedges)-1):
            txt = np.array2string(hist[i,j].astype(int))
...
def eval_performance(pred, gt, result_path, thres, force_scaler):
    curr_result_path = f"{result_path}/1probe"
    if not os.path.exists(curr_result_path):
        os.makedirs(curr_result_path)
    eval_performance_single_probe_situation(pred, gt, curr_result_path, thres, force_scaler, probes=[0,1])

    curr_result_path = f"{result_path}/2probe"
    if not os.path.exists(curr_result_path):
        os.makedirs(curr_result_path)
    eval_performance_single_probe_situation(pred, gt, curr_result_path, thres, force_scaler, probes=[0,2])

    curr_result_path = f"{result_path}/3probe"
    if not os.path.exists(curr_result_path):
        os.makedirs(curr_result_path)
    eval_performance_single_probe_situation(pred, gt, curr_result_path, thres, force_scaler, probes=[0,3])

    curr_result_path = f"{result_path}/all_probe"
    if not os.path.exists(curr_result_path):
        os.makedirs(curr_result_path)
    eval_performance_single_probe_situation(pred, gt, curr_result_path, thres, force_scaler, probes=[0,1,2,3])

def eval_performance_single_probe_situation(pred, gt, result_path, thres, force_scaler, probes=[1,2,3]):
    ...
    # for i in range(500):
    # TODO: to remap the coordinates to corresponds to the real taxel location
    for i in range(100):
        # ax = fig.add_subplot(20, 25, i+1)
        ax = fig.add_subplot(10, 10, i+1)
        # ax.hist(err_matrix[:,i],10, color='blue',alpha=0.8)
        ax.boxplot(err_matrix_real[:,i])
        ax.set_title('median: '+ str(np.around(np.median(err_matrix_real[:,i]),3)) + ', max: ' + str(np.around(np.max(err_matrix_real[:,i]),3)))    
    plt.draw()
    plt.savefig(f"{result_path}/force_error_all_taxel_wise.png", dpi=400, bbox_inches='tight')
    plt.close()
    ...
    for i in range(100):
        ...
        ax = fig.add_subplot(10, 10, i+1)
        # ax.hist(err_matrix[:,i],10, color='blue',alpha=0.8)
        curr_err_matrix_real = err_matrix_real[:,i]
        curr_gt = gt[:,i]
        curr_gt_true_inds = (curr_gt > thres).nonzero()
        if len(curr_gt_true_inds[0]) > 0:
            curr_err_matrix_real_nonzero = curr_err_matrix_real[curr_gt_true_inds]
            ax.boxplot(curr_err_matrix_real_nonzero)
            ax.set_title('median: '+ str(np.around(np.median(curr_err_matrix_real_nonzero),3)) + ', max: ' + str(np.around(np.max(curr_err_matrix_real_nonzero),3)))    
    plt.draw()
    plt.savefig(f"{result_path}/force_error_all_taxel_wise_nonzero.png", dpi=400, bbox_inches='tight')
    plt.close()
...
def infer_with_minibatch(model: torch.nn.Module, 
    ...
    # print(">>> Inferred {} samples.".format(len(preds)))
...
def analyze_raw_hall_and_force_signals(hall_statistics, mAP_loc_wise, hall_scaler, force_scaler, result_path):
    ...
    # vis_matrix(loc_wise_hall_QHull_vol, range(1,11), range(1,11), "Location-wise hall signal range", f"{result_path}/hall_range_loc_wise_QHullVol.png")
    # vis_matrix(loc_wise_hall_scaled_QHull_vol, range(1,11), range(1,11), "Location-wise hall signal range", f"{result_path}/hall_range_loc_wise_scaled_QHullVol.png")
...
def evaluate_models(cfgs):
    ...
    # need scaler to rescale to real values
    # train_dataloader = DataLoader(training_data, batch_size=batch_size, drop_last=False, shuffle=False)
    # val_dataloader = DataLoader(val_data, batch_size=batch_size, drop_last=False, shuffle=False) 
    test_data_known_dataloader = DataLoader(test_data_known, batch_size=batch_size, drop_last=False, shuffle=False)
    if test_data_novel is not None:
        test_data_novel_dataloader = DataLoader(test_data_novel, batch_size=batch_size, drop_last=False, shuffle=False)  
    ...
    # pred_train, Y_train = infer_with_minibatch(model, 
    #           train_dataloader, 
    #           device)
    # pred_val, Y_val = infer_with_minibatch(model, 
    #           val_dataloader, 
    #           device)
    pred_test_known, Y_test_known = infer_with_minibatch(model, 
              ...
              device)
    if test_data_novel is not None:
        pred_test_novel, Y_test_novel = infer_with_minibatch(model, 
                test_data_novel_dataloader, 
                device)

    evaluate_result_path = f"{result_path}/eval_{faces}"
    ...
    testset_path = f"{evaluate_result_path}/test_known"
    if not os.path.exists(testset_path):
        os.makedirs(testset_path)
    eval_performance(pred_test_known, Y_test_known, testset_path, thres, force_scaler)

    if test_data_novel is not None:
        testset_path = f"{evaluate_result_path}/test_novel"
        if not os.path.exists(testset_path):
            os.makedirs(testset_path)
        eval_performance(pred_test_novel, Y_test_novel, testset_path, thres, force_scaler)
